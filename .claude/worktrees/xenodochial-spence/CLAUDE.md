...
4. Set Anthropic and Gemini API keys to work with Plano — config only has sentinel-openai provider routing through proxy; no sentinel-anthropic or sentinel-gemini provider. Investigate Anthropic API failures (34% failure rate, malformed /anthropic/v1/messages path suggests ANTHROPIC_BASE_URL double-prefix).
5. Set up sentinel memory plugin. Add memory operation test scenarios — Invariants 4 (size caps) and 5 (no credentials in memory) are completely unexercised in production. OpenClaw memory isolation — separate memory systems (Phase 3)
6. Fix set-up and create guide to make it more intuitive and user friendly
7. Execution delegation via upstream hook change — propose `result` field on `BeforeToolCallResult` to OpenClaw (spec: `docs/superpowers/specs/2026-03-16-execution-delegation-design.md`). Interim: 6-layer advisory defense (classify + confirm + output redact + message redact + network isolation + credential isolation)
8. Update SOUL.md and AGENTS.md so that openclaw knows what sentinel is aware of it. Keep the level of detail 'as needed' so it can't modify sentinel settings
...
13. Custom undici dispatcher | Full content visibility through CONNECT tunnels (follow-up to domain-level CONNECT proxy). Placeholder injection at proxy.
...
- **Classifier unknown tool default** — unclassified tools use `inferCategoryFromName()` heuristic (read/write token matching on tool name, write wins); explicitly classified tools still go in `packages/policy/src/rules.ts` `DEFAULT_CLASSIFICATIONS` AND `config/default-classifications.json`
...
| Guard SDK + CONNECT Proxy | — | #33 | `@sentinel/guard-client` SDK (23 tests), CONNECT tunnel proxy handler (22 tests), Docker HTTPS_PROXY wiring, gateway entrypoint prep |
| Low-Hanging Fruit Batch | — | #34 | `parameters_summary` audit forensics (redact-before-truncate), `inferCategoryFromName` classifier heuristic, TIER_CONSTRAINTS conversational tone, SOUL.md fallback template, rate limiter stress tests |
